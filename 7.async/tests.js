describe('Домашнее задание к лекции 7 «Асинхронность»', () => 
  ...
  it('будильник должен удалять звонки по времени', () => 
  {
    ...
    clock.addClock("12:15", callback);
    clock.addClock("12:15", callback);
    clock.addClock("12:16", callback);
    clock.removeClock("12:15");
    expect(clock.alarmCollection.length).toEqual(1);
    expect(clock.alarmCollection).toEqual([{time: "12:16", callback, canCall: true}]);
  });
  ...
  {
    expect(() => clock.addClock("12:15")).toThrow();
  });
  ...
  {
    clock.addClock("12:15", f => f);
    clock.getCurrentFormattedTime = () => "12:30";
    clock.start();
  ...
  it('будильник должен запускать интервал, который запустит колбек', (done) => 
  {
    ...
    clock.addClock("12:15", () => flagToCall = true);
    clock.getCurrentFormattedTime = () => "12:15";
    clock.start();
